...
export default function Home(){
 ...
 useEffect(()=>{const s=localStorage.getItem("rt-score");if(s)setScore(JSON.parse(s));const v=localStorage.getItem("rt-speech");if(v!==null)setSpeech(v==="true");if("serviceWorker"in navigator)navigator.serviceWorker.register("/sw.js",{updateViaCache:"none"}).then(r=>r.update()).catch(()=>{})},[]);
 const speak=(text:string)=>{if(!speech||!("speechSynthesis"in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.rate=.92;window.speechSynthesis.speak(u)};
 useEffect(()=>{if(screen!=="live")return;const receive=(event:Event)=>{const detail=(event as CustomEvent).detail as {type:string;status?:"tracking"|"off-route";index?:number;nextIndex?:number;finished?:boolean};if(detail.type==="status"&&detail.status)setLive(detail.status);if(detail.type==="off-route")speak(`You are off Route ${route.number}. Return to the route when safe.`);if(detail.type==="back-on-route")speak(`You are back on Route ${route.number}.`);if(detail.index==null)return;const current=dir.steps[detail.index];if(!current)return;if(detail.type==="prepare")speak(`Prepare to ${label(current.action).toLowerCase()} on ${current.street}.`);if(detail.type==="near")speak(`${label(current.action)} on ${current.street}.`);if(detail.type==="complete"){const nextIndex=detail.nextIndex??detail.index;setIndex(Math.min(nextIndex,dir.steps.length-1));const following=dir.steps[nextIndex];speak(detail.finished?`Route complete. You have arrived at ${dir.destination}.`:`Turn completed. Next, ${label(following.action).toLowerCase()} on ${following.street}.`)} };window.addEventListener("route-trainer-gps",receive);return()=>window.removeEventListener("route-trainer-gps",receive)},[screen,speech,route.number,dir]);
