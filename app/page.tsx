...
export default function Home(){
 ...
 useEffect(()=>{if(screen!=="live")return;const receive=(event:Event)=>{const detail=(event as CustomEvent).detail as {type:string;status?:"tracking"|"off-route";index?:number;nextIndex?:number;finished?:boolean;stopName?:string};const prefix=`${route.id}:${dir.id}`;if(detail.type==="status"&&detail.status)setLive(detail.status);if(detail.type==="start")speak("GPS guidance started. Locating you on the route.",{interrupt:false,key:`${prefix}:start`});if(detail.type==="gps-weak")speak("GPS accuracy is weak. Holding navigation instructions until the location improves.",{key:`${prefix}:gps-weak`});if(detail.type==="off-route")speak(`You are off Route ${route.number}. Return to the highlighted route when safe.`,{key:`${prefix}:off-route`});if(detail.type==="back-on-route")speak(`You are back on Route ${route.number}.`,{key:`${prefix}:back-on-route`});if(detail.type==="wrong-way")speak(`You are traveling opposite the selected ${dir.label} direction. Turn around when safe.`,{key:`${prefix}:wrong-way`});if(detail.type==="direction-correct")speak(`Direction corrected. Continuing ${dir.label} guidance.`,{key:`${prefix}:direction-correct`});if(detail.type==="stop-ahead"&&stopAlertsRef.current&&detail.stopName)speak(`Bus stop ahead in 300 feet. ${detail.stopName}.`,{interrupt:false,key:`${prefix}:stop:${detail.stopName}`});if(detail.index==null)return;const current=dir.steps[detail.index];if(!current)return;const key=`${prefix}:${detail.index}`;if(detail.type==="acquired"){setIndex(detail.index);speak(`Location found. Next, ${spokenManeuver(current)}.${current.note?` ${current.note}.`:""}`,{key:`${key}:acquired`})}if(detail.type==="reacquired"){setIndex(detail.index);speak(`Travel direction confirmed. Next, ${spokenManeuver(current)}.`,{key:`${key}:reacquired`})}if(detail.type==="prepare")speak(`In about half a mile, prepare to ${spokenManeuver(current)}.`,{key:`${key}:prepare`});if(detail.type==="near")speak(`In about 600 feet, ${spokenManeuver(current)}.`,{key:`${key}:near`});if(detail.type==="now")speak(`Now, ${spokenManeuver(current)}.`,{key:`${key}:now`});if(detail.type==="complete"){const nextIndex=detail.nextIndex??detail.index;setIndex(Math.min(nextIndex,dir.steps.length-1));const following=dir.steps[nextIndex];speak(detail.finished?`Route complete. You have arrived at ${dir.destination}.`:`Turn completed. Next, ${spokenManeuver(following)}.${following.note?` ${following.note}.`:""}`,{key:`${key}:complete`})}};window.addEventListener("route-trainer-gps",receive);return()=>window.removeEventListener("route-trainer-gps",receive)},[screen,speak,route.id,route.number,dir]);
 const announceStep=(i=index)=>{const s=dir.steps[i];speak(`${label(s.action)} on ${s.street}. Continue to ${s.until}.${s.note?` ${s.note}.`:""}`,{key:`manual:${route.id}:${dir.id}:${i}:${Date.now()}`})};
